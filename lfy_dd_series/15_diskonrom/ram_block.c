...
static int rb_open(struct block_device *bdev, fmode_t mode)
{
	...
	printk(KERN_INFO "rb(rb_open_fn): Device is opened\n");
...
{
	int ret;
	printk("(S1)first step-1 \n");
	/* Set up our RAM Device */
	if ((ret = ramdevice_init()) < 0)
	{
		return ret;
	}
	printk("(S2)Ramdevice initialiation success-ret value=%d \n",ret);
	rb_dev.size = ret;
	...
	printk("(S3)register blk device(rb) success-step3 \n");
	...
	printk("(S4)queue creation success(rb_dev.rb_queue) success-step3 \n");
